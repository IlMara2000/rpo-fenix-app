...
export default function Home() {
  ...
  const handleDividerSubmit = async (e) => {
    ...
    setLoading(true);
    setStatus({ msg: 'DIVISIONE FILE RPO IN CORSO...', type: 'gold' });
    try {
    ...
    <div className="min-h-screen flex flex-col items-center py-12 px-6 text-white" 
      ...
      <header className="w-full max-w-4xl mb-12 flex flex-col items-center">
        ...
        <div className="status-badge shadow-2xl shadow-black/40 border-white/20 bg-black/60 backdrop-blur-xl">
          <span className="dot" style={{ backgroundColor: '#D4AF37' }}></span>
      ...
      <main className="w-full max-w-[1400px] grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
        ...
        {/* STEP 1: CONVERTER - Hover Gold */}
        <section className="box-lavoro relative overflow-hidden group h-full border-white/10 bg-black/40 backdrop-blur-md transition-all duration-300 hover:border-[#D4AF37]/50 hover:shadow-[0_0_30px_rgba(212,175,55,0.15)]">
          <div className="absolute -top-6 -right-4 text-9xl font-black text-white/[0.03] select-none group-hover:text-[#D4AF37]/[0.05] transition-colors">
            01
          </div>
          <h2 className="text-2xl font-bold mb-3 flex items-center gap-3">
            <span className="w-10 h-10 rounded-xl bg-gradient-to-br from-[#D4AF37] to-[#AA8418] flex items-center justify-center text-black text-base font-black shadow-lg">
              1
            </span>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white/[0.05] p-4 rounded-2xl border border-white/[0.1] flex items-center gap-3">
              <label className="px-4 py-2 rounded-xl text-xs font-bold uppercase text-black shadow-md cursor-pointer transition-all hover:brightness-110 active:scale-95" 
                     style={{ background: 'linear-gradient(to bottom, #FFD700, #D4AF37)' }}>
                <input
                  type="file"
                  onChange={e => {
            ...
            <button
              ...
              className="w-full py-4 rounded-2xl text-base font-black tracking-widest uppercase text-black shadow-xl transition-all hover:scale-[1.02] active:scale-95"
              style={{ background: 'linear-gradient(135deg, #FFD700 0%, #D4AF37 50%, #AA8418 100%)' }}
            >
            ...
            {converterFiles && (
              <div className="grid grid-cols-2 gap-4 animate-in fade-in">
                <button onClick={() => saveAs(converterFiles.txt, `perinvio${converterFiles.fileName}.txt`)} className="bottone-download text-[10px] border-[#D4AF37]/50 text-[#D4AF37] bg-[#D4AF37]/5">⬇️ TXT ⬇️</button>
                <button onClick={() => saveAs(converterFiles.zip, `perinvio${converterFiles.fileName}.zip`)} className="bottone-download text-[10px] font-bold" style={{ background: '#D4AF37', color: 'black' }}>📦 ZIP 📦</button>
              </div>
            )}
          </div>
        </section>

        {/* STEP 2: DIVIDER - Hover Gold */}
        <section className="box-lavoro relative overflow-hidden group h-full border-white/10 bg-black/40 backdrop-blur-md transition-all duration-300 hover:border-[#D4AF37]/50 hover:shadow-[0_0_30px_rgba(212,175,55,0.15)]">
          <div className="absolute -top-6 -right-4 text-9xl font-black text-white/[0.03] select-none group-hover:text-[#D4AF37]/[0.05] transition-colors">
            02
          </div>
          ...
          <form onSubmit={handleDividerSubmit} className="space-y-6">
            ...
            {dividerFiles && (
              <div className="pt-6 border-t border-white/20 mt-4 grid grid-cols-2 gap-4 animate-in fade-in">
                <button type="button" onClick={() => saveAs(dividerFiles.txtUno, `rpo_1_${dividerFiles.fileName}.txt`)} className="bottone-download text-[10px] py-3 border-[#D4AF37]/50 text-[#D4AF37] bg-[#D4AF37]/5">📄 RPO (1)</button>
                <button type="button" onClick={() => saveAs(dividerFiles.txtZero, `rpo_0_${dividerFiles.fileName}.txt`)} className="bottone-download text-[10px] py-3 font-bold" style={{ background: '#D4AF37', color: 'black' }}>📄 OK (0)</button>
              </div>
            )}
          </form>
        </section>
