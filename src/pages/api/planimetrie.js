...
import sharp from 'sharp'; // Libreria per il Watermark
...
export default async function handler(req, res) {
  ...
  form.parse(req, async (err, fields, files) => {
    ...
    try {
      ...
      console.log(`📡 Richiesta Photoreal al Mac M4 (Stile: ${styleChoice})...`);
      ...
      if (fs.existsSync(logoPath)) {
        finalBuffer = await sharp(baseImageBuffer)
          .composite([{ 
            ...
            gravity: 'southeast', 
            blend: 'over',
            // Riduciamo leggermente il logo se troppo grande (opzionale)
            // .resize({ width: 200 }) 
          }])
          .toBuffer();
      } else {
        console.warn("⚠️ Logo non trovato in public/logo.png - Procedo senza watermark.");
